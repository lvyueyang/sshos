 ...
 * 复用 services/capabilities 的 distro-profile 探测与缓存，executor 基于 ssh2 exec 通道。
 * 探测命令为服务端固定只读命令，走直接 exec（不经策略分类），与 metrics 采样一致；
 * 工具名经白名单字符集校验，不接受用户任意文本。
 */

import { execWithPolicy } from "../../ssh/command/exec.service";
import { execCommand } from "../../ssh/connection/ssh.server";
import {
	type DistroExecutor,
	type DistroProfile,
	getOrDetectDistroProfile,
} from "../distro/distro-profile";
import { TOOL_CACHE_TTL_MS, toolCache } from "../tools/cache";
import {
	buildInstallCommand,
	INSTALL_KNOWLEDGE,
