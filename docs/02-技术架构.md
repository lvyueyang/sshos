...
> **为什么不是 SFn async generator / 返回 Response**：早期 `createServerFn` 的 handler 返回值被 `startSerializer` 序列化，async generator 无法流式返回（旧结论，D21 后已被官方 RawStream 帧协议推翻，SFn 支持流式）；SFn 返回 `Response` 实测被序列化层吞掉（200 空 body）。AI 对话最终走 Server Route `/api/ai/chat`（SSE）——该选择仍成立（SSE 事件帧语义 + 现有客户端实现），也可迁移为 SFn 流式，两种范式并存。
