...
import { auditLogWriter, type LogAction, type LogResult } from "#/services/logs/audit/audit-writer.server";
...
AI / 自动操作的命令路径在 `exec.service.ts` 内联落审计（`execWithPolicy` 内 `writeAudit`：block / review / safe 全记录），写 SQLite `log` 表（`type` = `policy_decision`，含 `classification` / `action` / `result`）。高频写入用审计日志专属 BatchWriter 实例（`services/logs/audit/audit-writer.server.ts` 的 `auditLogWriter`，底层为 `lib/batch-writer` 通用 `BatchWriter<T>`，定时/定量批量 INSERT + 容量上限 + 进程退出时强制刷新 + 失败退避重试不丢审计）。用户手动操作不落策略审计（终端命令由 `recordTerminalCommandSFn` 单独记录）。
