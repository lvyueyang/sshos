...
用户输入 → useChat (ai-react) → aiChatSFn (POST + stream)
  ...
  → Pi SDK session.prompt() + session.subscribe() 逐 token 产出
  → 客户端流式渲染
...
export const aiChatSFn = createServerFn({ method: 'POST', response: 'raw' })
  ...
  .handler(async ({ data, context }) => {
    ...
    // session.subscribe 事件流 → SSE 帧；prompt 触发一轮生成（0.84.2 实测 API）
      ...
      start(controller) {
        agent.subscribe((event) => {
          ...
          if (event.type === 'tool_call') {
            controller.enqueue(encodeSse({ type: 'tool_call', content: event.toolName }))
          }
    ...
    void agent.prompt(data.messages.map((m) => m.content).join('\n'))
...
// 工具 execute 由调用方（ai.functions.ts）以回调注入
export async function createPiAgent(config: AgentConfig) {
  const shellTool = defineTool({
    name: 'shell',
    label: 'Shell',
    description: '在远程会话执行 shell 命令',
    parameters: Type.Object({ command: Type.String() }),
    execute: async (_toolCallId, { command }) => {
      return config.tools.execCommand({ command })   // → { isError, content: AgentContent[] }
    },
  })
  ...
  const fileReadTool = defineTool({
    ...
    execute: async (_toolCallId, { path }) => {
      const text = await config.tools.readFile({ path })
      return { isError: false, content: [{ type: 'text', text }] }
    },
  })

  const { session } = await createAgentSession({
    noTools: 'all',                    // 禁用内置工具，只暴露自定义工具
    customTools: [shellTool, fileReadTool],
  })
...
```typescript
// packages/web/src/apps/ai/ai.functions.ts —— 注入侧：把 SFn 包装成工具 execute
const agent = await createPiAgent({
  sessionId: data.sessionId,
  userId: context.userId,
  tools: {
...
Electron main 启动流程：
  ...
  2. 启动打包后的 react-start server（Nitro 产物 `.output/server/index.mjs`，Electron main spawn + `/api/health` 自检）
  3. 注册 ssh:// 协议处理器
...
| HTTP 服务启动 | react-start Nitro 打包（`.output/server/index.mjs`，Electron main spawn + health 自检） |
