...
│       │   │   ├── batch-writer/    # 审计日志批量写入（batch-writer.server.ts，仅服务端）
...
AI / 自动操作的命令路径在 `exec.service.ts` 内联落审计（`execWithPolicy` 内 `writeAudit`：block / review / safe 全记录），写 SQLite `log` 表（`type` = `policy_decision`，含 `classification` / `action` / `result`）。高频写入用 BatchWriter 缓冲（对齐 fsdx：定时/定量批量 INSERT + 容量上限 + 进程退出时强制刷新）。用户手动操作不落策略审计（终端命令由 `recordTerminalCommandSFn` 单独记录）。
