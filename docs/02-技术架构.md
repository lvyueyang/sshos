...
export const connection = sqliteTable('connection', {
  ...
  passwordEnc: text('password_enc'),    // master.key 加密（D21）
  privateKeyEnc: text('private_key_enc'),   // 手动粘贴的私钥，master.key 加密
  privateKeyPath: text('private_key_path'),   // 系统密钥路径，如 ~/.ssh/id_ed25519
  passphraseEnc: text('passphrase_enc'),      // 私钥密码，master.key 加密
...
| --- | --- | --- | --- |
| 密码 | `password` | `password_enc`（master.key 加密） | 直接用密码认证 |
| 手动私钥 | `privateKey` | `private_key_enc`（master.key 加密） | 用粘贴的私钥内容认证 |
...
密码、私钥、passphrase 加密后存入 SQLite，不以明文落盘。实现为 **AES-256-GCM + 数据目录 master.key**（决策记录 D21，取代 D18 密钥桥接）：首次启动 `POST /api/auth/setup` 时生成随机主密钥落盘 `{dataDir}/master.key`（32 字节 hex，0600），`db/crypto.ts` 以其派生 AES-256 主密钥加解密凭据。主密钥独立于启动密码（改启动密码不触发凭据重加密），桌面壳与纯 web 模式同构（不再依赖 Electron safeStorage / env 注入）。

```typescript
// packages/web/src/services/auth/config.ts（web 服务自洽，setup 时生成）
export function getOrCreateMasterKeyFile(): Buffer {
  // {dataDir}/master.key 存在则读回，否则 randomBytes(32) 写盘（0600）
}

// packages/web/src/db/crypto.ts
const secret = getOrCreateMasterKeyFile() // 派生 AES-256 主密钥
```

`setCredentialEncryptor` 保留为宿主注入点（如未来 Electron safeStorage 代理）。生产环境 master.key 缺失或不可读时 **fail-fast**（启动/加密即抛错），不降级明文；仅非生产环境降级公开密钥（等价明文）供开发验证。
...
| 敏感数据 | 密码/私钥已通过 master.key 加密，同步的是加密后的密文，不在远端暴露明文 |
...
interface WebDAVConfig {
  ...
  username: string
  passwordEnc: string  // master.key 加密
...
web server 启动流程（server.ts / Nitro entry）：
  ...
  2. runMigrations() + runSeed()（fail-fast，失败即退出）
  3. SIGTERM/SIGINT → batchWriter.flushOnExit()（优雅关闭刷审计缓冲）
  4. 未配置（server.json 无 passwordHash）→ 仅放行 /api/auth/* 与 /api/health、页面，业务请求 401
...
| 认证（setup/login/status）、JWT 签发、server.json / master.key | `packages/web/src/services/auth` + `routes/api/auth/*` |
...
> **D21（web 服务模式）**：认证走「启动密码登录签发 JWT」，客户端 token 存 localStorage（`lib/auth-client.ts`），全局 `middleware/auth.ts` 校验 `X-SSHOS-TOKEN`；`/api/deeplink` 路由已删除（深链改为壳的 URL 参数职责）。D18 密钥桥接与 D19 env token 机制废止，详见决策记录。
