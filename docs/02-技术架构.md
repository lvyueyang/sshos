...
| `connection_setting` | 每连接配置（App 会话状态 / 桌面布局快照） |
...
1. **服务端会话状态**（内存态）：SSH / PTY / SFTP 会话以 `sessionId` 为 key 的内存 Map 维护，生命周期绑定 Tab，连接断开即清理，**不持久化**；仅连接配置落 SQLite。
2. **客户端数据缓存**：TanStack Query 管理连接 / 分组 / 设置等数据，`queryKey` 映射 SFn + 入参，增删改后 `invalidateQueries`。
3. **客户端 UI 状态**（Zustand）：Tab 列表、窗口管理器、焦点、主题偏好（持久化到 `setting` key = `appearance.theme`）。**流式数据（PTY 输出、监控指标、AI 对话）不进全局 store**，由组件内局部 hook 直接消费。
4. **App 实例状态**：`app-manager` 读写 `connection_setting`（key = `desktop.layout` / `app.<id>.state`），负责生命周期保存/恢复分发（见 §6）。
...
```text
浏览器 Terminal（xterm.js）  ⇄  WebSocket   ⇄  web 服务 PTY 网关  ⇄  ssh2 PTY channel  ⇄  远程 shell
   onData ──上行帧 input/resize ──▶              PTY.write / PTY.resize
   Terminal.write ◀──下行帧 output──              PTY stdout 数据
```

协议栈：

- **握手**：客户端先经 `ptyWsTicketSFn` 获取一次性握手票据（绑定 sessionId），再以票据建立 WebSocket 连接；服务端按票据校验鉴权并绑定 PTY 会话。
- **下行帧**（Server → Client）：`{ type: 'output', data }`，客户端解码后写入 `Terminal.write`。
...
```
连接（connectSFn）→ 会话（sessionId）→ 打开终端窗口 createPtySFn → WebSocket 通道（ptyWsTicketSFn）
  → 交互（output/input/resize） → 窗口关闭 closePtySFn → Tab 关闭 disconnectSFn（清审批挂起 / Profile / 工具缓存）
```
...
- `app-manager`：每 Tab（连接）一个实例，负责生命周期分发与布局快照读写；**纯客户端实现**——settings / audit / log 经构造注入（SFn 网关），不直连服务端模块。
...
- **app 偏好配置**（跨连接）→ 全局 `setting` key = `app.<id>.*`
