...
│       │   │   ├── http-guard/      # 全局鉴权豁免判定 isProtected（通用）
│       │   │   ├── logger/          # Pino 日志工厂（logger.server.ts，仅服务端）
│       │   │   ├── logs-format/     # 日志展示格式化（渲染层，isomorphic-safe）
│       │   │   ├── paths/           # 数据目录解析（paths.server.ts，仅服务端）
│       │   │   ├── public-sfns/     # 公开 SFn 注册表（通用）
...
│       │   │   │   ├── auth.functions.ts #   公开 SFn（registerPublicSfn 豁免）
│       │   │   │   └── auth.schemas.ts
