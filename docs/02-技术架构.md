...
> **W4 前置（凭据加密接线）**：当前凭据加密默认走 `SSHOS_MASTER_KEY` 派生或公开 `dev-only-master-key` 降级密钥（等价明文，仅开发验证期接受）。W4 打包前必须在 web server 启动时注入 `setCredentialEncryptor`（Electron `safeStorage` 实现，macOS Keychain / Windows DPAPI / Linux libsecret），消除降级路径。因 Nitro server 是独立 Node 子进程（无 Electron API），接线方式需 W4 定案（进程内启动或密钥桥接）。
