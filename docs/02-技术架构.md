...
密码、私钥、passphrase 加密后存入 SQLite，不以明文落盘。实现为 **AES-256-GCM + 数据目录 master.key**（决策记录 D21，取代 D18 密钥桥接）：首次启动 `setupSFn` 时生成随机主密钥落盘 `{dataDir}/master.key`（32 字节 hex，0600），`db/crypto.ts` 以其派生 AES-256 主密钥加解密凭据。主密钥独立于启动密码（改启动密码不触发凭据重加密），桌面壳与纯 web 模式同构（不再依赖 Electron safeStorage / env 注入）。
...
| 认证（setup/login/status）、JWT 签发、server.json / master.key | `packages/web/src/services/auth`（auth.functions 公开 SFn + auth.server） |
