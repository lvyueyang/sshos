...
│       │       ├── use-pty-stream.ts    # PTY 流消费（docs 技术架构 §5.2）
...
密码、私钥、passphrase 加密后存入 SQLite，不以明文落盘。实现为 **AES-256-GCM + master key 桥接**（决策记录 D18）：master key 由 Electron main 用 `safeStorage` 生成并加密持久化（`userData/master.key`，macOS Keychain / Windows DPAPI / Linux libsecret），启动 Nitro 子进程时经 `SSHOS_MASTER_KEY` 环境变量注入；`db/crypto.ts` 用其派生 AES-256 主密钥加解密凭据。`setCredentialEncryptor` 为未来 headless 服务模式（进程内启动 Nitro）预留的注入点。

```typescript
// packages/desktop/src/secure-key.ts（Electron main，safeStorage 可用）
export function getOrCreateMasterKey(): string | null {
  if (!safeStorage.isEncryptionAvailable()) return null
  // userData/master.key 存在则解密读回，否则生成随机 key 加密落盘
}

// packages/web/src/db/crypto.ts（Nitro 子进程）
const secret = process.env.SSHOS_MASTER_KEY // main 注入
const key = createHash('sha256').update(secret).digest() // AES-256 主密钥
```

safeStorage 不可用（如 Linux 无 keyring）时降级公开密钥并双侧告警（main 与 crypto），仅开发验证期与无 keyring 环境接受。
...
桌面窗口状态是纯客户端全局态，由 Zustand store 管理：Tab 列表、窗口管理器（z-index / position / size / minimized / maximized）、聚焦窗口、主题偏好（light/dark，运行时走 store，持久化到 `setting` 表 key = `appearance.theme`）。store 只存 UI 状态，不存远程数据。**流式数据（PTY 输出、监控指标、AI 对话）不进全局 store**——高频更新会触发整树重渲染，由组件内局部 hook（`use-pty-stream`、`use-metrics-stream`、AiPanel 内联 SSE 消费）直接消费。
...
| AI 对话流 | Server Route POST（SSE） | `server.handlers.POST` 返回 SSE `Response`，AiPanel 手动 `fetch` 消费 |
...
**AI 对话流** — 走 Server Route `POST /api/ai/chat`（handler 返回 SSE `Response`，服务端依赖在 handler 内动态 import），客户端 `AiPanel` 手动 `fetch` 并解析 `data:` 帧（`text-delta`）。`@tanstack/ai-react` 的 `useChat` 未采用（依赖保留）；SFn 返回 `Response` 会被 `startSerializer` 序列化层吞掉（实测 200 空 body），详见决策记录技术验证记录「SFn 返回 Response（SSE）」与 §8。
...
| `createConnectionSFn` | POST | 新建连接（凭据加密入库） | 同上 |
...
> AI 对话不走 SFn：`POST /api/ai/chat` Server Route 返回 SSE 流（见 §5.5 / §8），`@tanstack/ai-react` 的 `useChat` 未采用，AiPanel 手动消费 SSE（决策记录技术验证记录）。
...
| `api/sftp/download.tsx` | GET (stream) | SFTP 文件下载（ReadableStream，RFC 6266/5987 文件名） |
| `api/sftp/upload.tsx` | POST (stream) | SFTP 文件上传（body 直写远程路径） |
| `api/ai.chat.tsx` | POST (stream) | AI 对话流（SSE，text-delta 帧） |
...
## 8 AI 集成：Pi SDK + Server Route SSE 流式

AI 能力通过 Pi SDK 嵌入，AI 对话流由 Server Route（`/api/ai/chat`）返回 SSE，客户端 `AiPanel` 手动 `fetch` 消费。Policy Engine 拦截 AI 生成的命令（`exec.service` 的 `execWithPolicy`）。

> **包名核实**：AI 引擎包为 `@earendil-works/pi-coding-agent`（npm 已确认存在，`exports['.']` 提供 types + import 可编程入口）。`@pi.dev/sdk` 不存在，文档/代码中严禁使用。

> **客户端消费方式（W3 定稿）**：SFn 返回 `Response` 会被 `startSerializer` 序列化层吞掉（实测 200 空 body），故 AI 对话改走 Server Route `/api/ai/chat`（handler 返回 SSE `Response` 是标准形态）；`@tanstack/ai-react` 的 `useChat` 未采用，`AiPanel` 手动解析 `data:` 帧（`text-delta`）。依赖保留但未使用（决策记录技术验证记录「SFn 返回 Response（SSE）」）。

### 8.1 集成架构

```text
用户输入 → AiPanel fetch POST /api/ai/chat
  → guardChatInput 注入检测（system 角色拒绝 + 注入模式）
  → Pi SDK session.prompt() + session.subscribe() 逐 token 产出
  → SSE（text-delta 帧）→ 客户端流式渲染
  → AI 工具调用 → exec.service.execWithPolicy → 分类 → 审批/拦截/执行
```

### 8.2 AI 对话服务端（Server Route）

`routes/api/ai.chat.tsx` 是 `defineServerRoute` 的 Server Route（服务端依赖动态 import，避免进入 client bundle），POST body = `{ sessionId, messages }`，返回 SSE 流。服务端逻辑在 `apps/ai/ai.chat.server.ts` 的 `createAiChatResponse`：
...
export async function createAiChatResponse(
  ...
  guardChatInput(messages)             // 注入检测：system 角色 + 注入模式

  const tools: AgentTools = {
    execCommand: async (command) => {
      try {
        const stdout = await execWithPolicy(sessionId, command)   // 命令经策略引擎
    ...
    // readFile / listDir 经 SFTP 读远程文件与列目录
  }

  const agent = await createPiAgent(tools)
  // session.subscribe 事件流 → SSE 帧（text_delta 增量 / text_end 兜底）
  // ...流控制器与关闭标记提升到流外：prompt 失败时也 closeSse()，避免客户端 loading 卡死
  void agent.prompt(messages.map((m) => m.content).join('\n'))
  return new Response(sse, { headers: { 'Content-Type': 'text/event-stream' } })
}
```

> **为什么不是 SFn async generator / 返回 Response**：`createServerFn` 的 handler 返回值会被 `startSerializer` 序列化，async generator 无法流式返回（源码验证，见决策记录 D2）；SFn 返回 `Response` 实测被序列化层吞掉（200 空 body）。正解是 Server Route `server.handlers` 返回 `new Response(ReadableStream)`，框架原样透传响应体。

### 8.3 客户端消费

```typescript
// packages/web/src/apps/ai/AiPanel.tsx —— 手动 fetch + 解析 SSE（未用 @tanstack/ai-react useChat）
const res = await fetch('/api/ai/chat', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ sessionId, messages: history }),
})
await consumeSse(res.body, (delta) => appendAssistantDelta(delta))
// consumeSse：按 \n\n 切帧，解析 data: JSON（{ type: 'text-delta', delta }）逐段追加
```
...
Electron main 启动流程：
  1. 单实例锁（ssh:// 深链依赖）→ app ready → bootstrap()
     ├─ 应用命名 + 注册 ssh:// 协议处理器（深链聚焦主窗口并暂存 URL）
     └─ 注：DB 迁移 / 预置数据 / 审计缓冲刷新由 web server 启动时执行（见下）
  2. 启动打包后的 react-start server（Nitro 产物 `.output/server/index.mjs`，Electron main spawn + `/api/health` 自检）
     ├─ server.ts 顶部 runMigrations() + runSeed()（fail-fast，失败即退出）
     └─ SIGTERM/SIGINT → batchWriter.flushOnExit()（优雅关闭刷审计缓冲）
  3. 加载 renderer（生产：打包产物；开发：vite dev server）
```

| 关注点 | 归属 |
| --- | --- |
| 单实例锁、ssh:// 协议注册、应用命名 | `packages/desktop/src/bootstrap.ts` |
...
> **W4 前置（凭据加密接线）**：已定案 **D18 密钥桥接**——Electron main 用 `safeStorage` 保护随机 master key（`userData/master.key`），经 `SSHOS_MASTER_KEY` 环境变量注入 Nitro 子进程派生 AES-256（见 §4.6）。main 生产启动同时注入 `NODE_ENV=production`，确保降级告警生效；safeStorage 不可用时降级并双侧告警。`setCredentialEncryptor` 保留为未来 headless 进程内启动的注入点。
