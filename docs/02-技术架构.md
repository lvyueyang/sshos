...
| 层级 | 技术 | 选型理由 |
| --- | --- | --- |
| 运行时 | Node.js（Electron 内置） | Electron 主进程内跑 react-start server（createServerEntry）；`node:sqlite` 依赖 Node 22.5+ |
...
| 终端渲染 | xterm.js + 官方 addon | `@xterm/addon-webgl` / `fit` / `search` / `web-links` / `unicode11` / `image`；不用 `addon-attach`（依赖 WebSocket） |
...
│       │   ├── server.ts         # 框架约定：SSR 入口（createServerEntry）
│       │   ├── client.tsx        # 框架约定：客户端水合（可选）
│       │   ├── routeTree.gen.ts  # ★ 自动生成，勿编辑
│       │   │
│       │   ├── app-framework/    # App 插件框架（manifest/context/生命周期分发）
│       │   │   ├── types.ts      # AppManifest / AppContext / Lifecycle 类型
│       │   │   ├── registry.ts   # 内置 app 静态聚合注册
│       │   │   ├── app-manager.ts    # 每 Tab 一个，驱动生命周期与布局快照
│       │   │   └── dispatcher.ts     # onCreate/onRestore/onSave/onShutdown 分发
│       │   │
│       │   ├── apps/             # 桌面应用（核心，顶层目录，每个 app 一个插件包）
│       │   │   ├── terminal/
│       │   │   │   ├── app.ts                # manifest + setup(ctx)
│       │   │   │   ├── index.tsx             # 终端窗口（window surface）
│       │   │   │   └── terminal.functions.ts # connect / sendInput / resizePty SFn
│       │   │   ├── files/
│       │   │   │   ├── app.ts                # manifest + setup(ctx)
│       │   │   │   ├── index.tsx             # 文件管理窗口
│       │   │   │   └── files.functions.ts    # sftpList / sftpStat / sftpMkdir / sftpDelete SFn
│       │   │   ├── monitor/
│       │   │   │   ├── app.ts                # manifest：window + panel 双 surface
│       │   │   │   ├── index.tsx             # 系统监控窗口
│       │   │   │   └── panel.tsx             # 桌面状态卡片（panel surface）
│       │   │   ├── clock/
│       │   │   │   └── app.ts                # statusbar surface · 自启
│       │   │   └── ai/
│       │   │       ├── app.ts                # manifest + setup(ctx)
│       │   │       ├── index.tsx             # AI 面板窗口
│       │   │       └── ai.functions.ts       # aiChat / execCommand SFn
│       │   │
│       │   ├── components/       # 通用桌面组件
│       │   │   ├── Sidebar.tsx   # 侧栏连接管理
...
│       │   │   ├── audit-log.ts       # 审计日志
│       │   │   └── prompt-guard.ts    # Prompt 注入检测（aiChatSFn）
│       │   │
...
│       │   │   ├── ssh/ssh.server.ts        # 连接生命周期（调 packages/core）
...
export const connection = sqliteTable('connection', {
  ...
  passwordEnc: text('password_enc'),    // Electron safeStorage 加密
  privateKeyEnc: text('private_key_enc'),   // 手动粘贴的私钥，safeStorage 加密
  privateKeyPath: text('private_key_path'),   // 系统密钥路径，如 ~/.ssh/id_ed25519
  passphraseEnc: text('passphrase_enc'),      // 私钥密码，safeStorage 加密
...
| --- | --- | --- | --- |
| 密码 | `password` | `password_enc`（safeStorage 加密） | 直接用密码认证 |
| 手动私钥 | `privateKey` | `private_key_enc`（safeStorage 加密） | 用粘贴的私钥内容认证 |
...
export function createSSHConnection(opts: ConnectionOpts) {
  ...
  switch (opts.authType) {
    case 'password':
      config.password = decrypt(opts.passwordEnc)
      break
    case 'privateKey':
      config.privateKey = decrypt(opts.privateKeyEnc)
      if (opts.passphraseEnc) {
        config.passphrase = decrypt(opts.passphraseEnc)
      }
      break
    case 'systemKey':
      // 实时读取系统密钥文件，不落库
...
密码、私钥、passphrase 通过 Electron `safeStorage` API 加密后存入 SQLite，不以明文落盘。safeStorage 在 macOS 上用 Keychain，Windows 上用 DPAPI，Linux 上用 libsecret。
...
| 平台 | 路径 |
| --- | --- |
| macOS | `~/Library/Application Support/ssh-os/ssh-os.db` |
| Windows | `%APPDATA%/ssh-os/ssh-os.db` |
| Linux | `~/.config/ssh-os/ssh-os.db` |

WAL 模式下同时存在 `ssh-os.db-wal` 和 `ssh-os.db-shm` 文件，属正常现象。
...
| 敏感数据 | 密码/私钥已通过 safeStorage 加密，同步的是加密后的密文，不在远端暴露明文 |
...
桌面窗口状态是纯客户端全局态，由 Zustand store 管理：Tab 列表、窗口管理器（z-index / position / size / minimized / maximized）、聚焦窗口、主题偏好（light/dark，运行时走 store，持久化到 `setting` 表 key = `appearance.theme`）。store 只存 UI 状态，不存远程数据。**流式数据（PTY 输出、监控指标、AI 对话）不进全局 store**——高频更新会触发整树重渲染，由组件内局部 hook（`use-pty-stream`、`useChat`）直接消费。
...
> **为什么不用 SFn async generator？**（已验证）TanStack Start 的 `createServerFn` 返回值经 `startSerializer.stringify` 序列化为 JSON，**无法序列化 async generator / 裸 ReadableStream**。流式场景的正解是 Server Route：handler 返回 `new Response(ReadableStream)`，框架原样透传响应体，客户端用 `fetch` 读 `response.body` 流。

### 5.1 通信模式总览

| 通信模式 | 承载形态 | 代码模式 |
| --- | --- | --- |
| PTY 输出流 | Server Route GET | `server.handlers.GET` 返回 `new Response(ReadableStream)` |
...
| 系统监控指标 | Server Route GET | 返回 `new Response(ReadableStream)`，每 2s 一个 chunk |
| AI 对话流 | Server Route / @tanstack/ai-react | `useChat` 消费 SSE 流 |
| 连接管理 | SFn POST + Zod | `.inputValidator(z).handler(sshManager.connect)` |
| 健康检查 | Server Route GET | `routes/api/health.tsx` 返回 JSON |

### 5.2 流式实现：Server Route + ReadableStream

**PTY 输出流** — 服务端把 PTY 的 Node Readable 转为 Web ReadableStream，放进 `Response` 返回；客户端 `fetch` 后用 `for await` 消费。适用于 PTY 输出、监控指标等持续下行流。

```typescript
// packages/web/src/routes/api/pty.$sessionId.tsx
import { Readable } from 'node:stream'
import { createFileRoute } from '@tanstack/react-router'
import { ptyManager } from '#/services/ssh/ssh.server'

export const Route = createFileRoute('/api/pty/$sessionId')({
  server: {
    handlers: {
      GET: async ({ params }) => {
        const pty = ptyManager.get(params.sessionId)
        const web = Readable.toWeb(pty.output) as ReadableStream<Uint8Array>
...
```typescript
// packages/web/src/routes/api/sftp.download.tsx —— 流式下载
...
**AI 对话流** — `@tanstack/ai-react` 的 `useChat` 原生支持 SSE 流式，Server Route 或 SFn handler 返回 SSE 响应体。若走 SFn 返回 `Response`，SFn 必须声明 `response: 'raw'`（否则返回值会被 `startSerializer` 序列化吞掉，见决策记录技术验证记录）。具体以 `@tanstack/ai-react` 0.19 的实际 connection API 为准（见决策记录待验证清单）。
...
终端是唯一的双向通信场景。用两条通道解决：一条 Server Route GET 流推送 PTY 输出，一条 SFn POST 发送键盘输入。
...
SFn 就近定义在消费它的应用组件目录下（`apps/<app>/<app>.functions.ts`，平级放置），变量名统一 `SFn` 后缀：

| SFn | HTTP 方法 | 说明 | 位置 |
| --- | --- | --- | --- |
| `connectSFn` | POST | 建立 SSH 连接，返回 sessionId | `apps/terminal/` |
| `disconnectSFn` | POST | 断开指定 SSH 连接 | 同上 |
| `listConnectionsSFn` | GET | 列出所有活跃连接 | 同上 |
| `sendInputSFn` | POST | 向 PTY 写入键盘输入 | 同上 |
| `resizePtySFn` | POST | 调整终端尺寸 | 同上 |
| `execCommandSFn` | POST | 执行单条命令（非交互式，挂策略） | `apps/ai/` |
| `approvalSFn` | POST | review 级命令审批决策（approved / rejected） | `approval/` |
| `sftpListSFn` | GET | 列出远程目录 | `apps/files/` |
| `sftpStatSFn` | GET | 获取文件元信息 | 同上 |
| `sftpMkdirSFn` | POST | 创建远程目录（低风险，不挂策略） | 同上 |
| `sftpDeleteSFn` | POST | 删除远程文件 / 目录（挂策略：review，敏感路径 block） | 同上 |
| `sftpRenameSFn` | POST | 重命名 / 移动（挂策略：review） | 同上 |
| `aiChatSFn` | POST | AI 对话流 | `apps/ai/` |
...
| `api/pty.$sessionId.tsx` | GET (stream) | PTY 输出流（ReadableStream） |
| `api/metrics.$sessionId.tsx` | GET (stream) | 系统指标流（每 2s 一个 chunk） |
| `api/sftp.download.tsx` | GET (stream) | SFTP 文件下载（ReadableStream） |
| `api/sftp.upload.tsx` | POST (stream) | SFTP 文件上传（ReadableStream） |
...
- `app-manager`：每 Tab（连接）一个实例，负责生命周期分发、surface 挂载、布局快照读写；渲染期窗口状态仍走 Zustand store
- **布局快照**（哪些 app 打开、各 surface 槽位、window bounds）→ `connection_setting` key = `desktop.layout`
- **app 会话状态**（`onSave()` 产物）→ `connection_setting` key = `app.<id>.state`
- **app 偏好配置**（如时钟格式，跨连接）→ 全局 `setting` key = `app.<id>.*`
- 系统退出时在优雅关闭阶段 flush 全部 `onSave()` 与审计缓冲（对齐 §7.8）
...
function classifyByReadWrite(cmd: string): Verdict {
  return READ_COMMANDS.some((re) => re.test(cmd))
    ? { level: 'safe', reason: 'read-only command' }
    : { level: 'review', reason: 'write / unknown command' }
}
...
// 文件操作分类：data = { sessionId, path }，删除/重命名/移动命中敏感路径 → block，
// 其余默认 review（SFTP 变更写操作需人工审批）；创建类（mkdir）由调用方按 safe 处理
...
export const baseRules = [
  // block 级
  { pattern: /rm\s+-rf?\s+\//, level: 'block', description: 'rm -rf root' },        // 覆盖 rm -rf / 与 rm -fr /
  { pattern: /rm\s+-r\s+-f\s+\//, level: 'block', description: 'rm -r -f root' },
...
策略引擎一次定义、全局复用。挂载到全部**写操作类 SFn**——命令执行（`command` kind）与 SFTP 变更写操作（`file` kind）；`sendInputSFn`（逐键流）与 `sftpMkdirSFn`（低风险创建）不挂载（见 §5.3）：

```typescript
// 写操作 SFn 共享审计 + 策略中间件
// 注意顺序：audit 必须在外层包裹 policy，才能捕获 policy 抛出的
// block/review 错误并落审计（policy 内 throw 前不调 next()）
import { policyEngineMiddleware, sftpPolicyMiddleware } from '../middleware/policy-engine'
import { auditLogMiddleware } from '../middleware/audit-log'

// 命令执行（shell 文本分类）
export const execCommandSFn = createServerFn()
  .middleware([auditLogMiddleware, policyEngineMiddleware])
  .inputValidator(commandSchema)
  .handler(async ({ data }) => {
    return sshManager.exec(data.sessionId, data.command)
  })
...
Prompt 注入检测与命令分类（Policy Engine）是两个维度，拆成独立中间件 `promptGuardMiddleware`，只挂在 AI 对话 SFn（`aiChatSFn`）上：

- **系统提示词隔离**：Pi 的 systemPrompt 由代码硬编码（`pi-agent.ts`），用户/AI 对话内容不得覆盖；对话 history 与系统指令分层存储、分层拼装。
- **用户输入边界**：用户消息作为数据传入（非指令），模型只被允许使用已注册工具；命令一律走 `tool_call` → `execCommandSFn`，不允许模型裸输出命令即执行。
- **输出后二次校验（纵深防御）**：即使 prompt 被注入，模型产出的命令进入执行前仍会被 `execCommandSFn` 的 Policy Engine 在 `.server()` 层再分类一次，block 级依旧被拦截。

```text
用户消息 → aiChatSFn (promptGuard 中间件)
  ├─ 检出注入（system 覆盖尝试 / 越权工具请求）→ 拒绝本次对话
  └─ 放行 → Pi 生成 tool_call → execCommandSFn (Policy Engine 再分类) → 审批/拦截/执行
```

## 8 AI 集成：Pi SDK + TanStack AI 流式

AI 能力通过 Pi SDK 嵌入，AI 对话流由 `@tanstack/ai-react` 在客户端消费。Policy Engine 作为 SFn 中间件自动拦截 AI 生成的命令。
...
```text
用户输入 → useChat (ai-react) → aiChatSFn (POST + stream)
  → promptGuardMiddleware 注入检测
  → Pi SDK agent.execute() 逐 token 产出
  → 客户端流式渲染
  → AI 工具调用 → execCommandSFn → Policy Engine 再校验 → 审批/拦截/执行
```

### 8.2 AI 对话 SFn

```typescript
// packages/web/src/apps/ai/ai.functions.ts
import { createServerFn } from '@tanstack/react-start'
import { z } from 'zod'
import { promptGuardMiddleware } from '#/middleware/prompt-guard'
import { createPiAgent } from '#/ai/pi-agent'

const chatSchema = z.object({
  // system 消息不允许客户端传入（见 §7.9 promptGuard 系统提示词隔离），系统指令只在服务端拼装
...
// 消费 Pi 流后返回 SSE Response（配合 @tanstack/ai-react 的 useChat）
// 注意：SFn 返回 Response 必须声明 response: 'raw'，否则被 startSerializer 序列化吞掉
export const aiChatSFn = createServerFn({ method: 'POST', response: 'raw' })
  .middleware([promptGuardMiddleware])   // 注入检测；命令安全由工具调用的 execCommandSFn 二次拦截
  .inputValidator(chatSchema)
  .handler(async ({ data, context }) => {
    const agent = createPiAgent({
      sessionId: data.sessionId,
      userId: context.userId,
      tools: ['shell', 'file_read', 'file_write', 'sftp'],
    })

    const stream = await agent.execute(data.messages)
    // Pi SDK 输出 chunk 类型：text | tool_call | tool_result
    return toSseResponse(stream, ({ type, content, toolCallId }) => ({
      type,
      content,
      toolCallId,
    }))
...
> **为什么不是 async generator**：`createServerFn` 的 handler 返回值会被 `startSerializer` 序列化，**async generator 无法直接流式返回**（源码验证，见决策记录 D2）。正确形态是 handler 内部消费 Pi 流，**返回 SSE `Response`**（SFn 需声明 `response: 'raw'`，或改走 Server Route），客户端 `useChat` 消费。`toSseResponse` 将 Pi chunk 转成 SSE 帧；最终细节在待验证清单中定稿。

### 8.3 客户端消费

```typescript
// packages/web/src/apps/ai/index.tsx
...
function AiPanel({ sessionId }: { sessionId: string }) {
  const chat = useChat({
    connection: stream((messages) =>
      aiChatSFn({ data: { messages, sessionId } })
    ),
  })
  // ...消息渲染、命令预览卡片、Policy 状态标签
}
```

> `stream()` 的确切签名以 `@tanstack/ai-react` 0.19 的 connection adapter API 为准（TanStack AI 已演进为 adapter 体系），spike 中核对后修正。

### 8.4 Pi Agent 封装

Agent 封装在 `packages/web/src/ai/pi-agent.ts`，负责将 Pi SDK 的工具调用映射到 SFn 函数：
...
// 依赖方向：pi-agent 不直接 import SFn（避免与 ai.functions.ts 循环依赖），
// 工具 handler 由调用方（ai.functions.ts）以回调注入
export function createPiAgent(config: AgentConfig) {
  const pi = new Pi({
    model: 'gpt-4o',
    systemPrompt: `You are a Linux system assistant.
Operating on session ${config.sessionId}.
Only use provided tools. Never output raw shell commands without tool_call.`,
  })

  // 注册工具 — handler 来自 config.tools（由 ai.functions.ts 把 SFn 包装后注入）
  pi.registerTool('shell', {
    description: 'Execute a shell command',
    parameters: { command: { type: 'string' } },
    handler: async ({ command }) => config.tools.execCommand({ command }),
  })

  pi.registerTool('file_read', {
    description: 'Read a remote file',
    parameters: { path: { type: 'string' } },
    handler: async ({ path }) => config.tools.readFile({ path }),
  })

  return {
    execute: (messages) => pi.stream({ messages }),
  }
}
```

```typescript
// packages/web/src/apps/ai/ai.functions.ts —— 注入侧：把 SFn 包装成工具 handler
const agent = createPiAgent({
  sessionId: data.sessionId,
  userId: context.userId,
  tools: {
    execCommand: ({ command }) =>
      execCommandSFn({ data: { sessionId: data.sessionId, command } }),
    readFile: ({ path }) =>
      fetch(`/api/sftp/download?sessionId=${data.sessionId}&path=${encodeURIComponent(path)}`).then((r) => r.text()),
  },
})
```

> **审批流转**：`execCommandSFn` 遇 review 级命令会抛 `ApprovalRequiredError`。`pi-agent` 捕获后将「需审批」状态编码进流式 chunk（`type: 'tool_awaiting_approval'`），客户端据此渲染审批卡片；用户决策后调 `approvalSFn` 重放执行，执行结果作为 `tool_result` 回喂模型。safe 命令同样经命令卡片确认后执行（见界面设计 §8.5）。

### 8.5 安全闭环

AI 生成的命令不会直接执行，而是通过 SFn 二次校验：

```text
用户: "清理 /var/log 下的旧日志"
  → aiChatSFn (promptGuard 注入检测)
  → Pi SDK 生成 tool_call: shell({ command: "find /var/log -mtime +30 -delete" })
  → execCommandSFn (Policy Engine 再次分类)
    → 命中 review 规则 (rm/delete)
    → 登记审批挂起表，抛 ApprovalRequiredError(requestId)
    → AI 命令卡片显示黄色，用户点 [批准执行]
    → approvalSFn(requestId, approved) → 重放执行
    → 结果返回给 AI → AI 告知用户
    → 用户点 [拒绝] → approvalSFn(requestId, rejected) → 丢弃，AI 告知已拒绝
```

> AI 的每一次工具调用都经过 SFn 中间件链，不存在"AI 直接执行命令"的路径。即使 Prompt 被注入，Policy Engine 仍会在 `.server()` 层拦截危险命令。

## 9 Electron 主进程与 bootstrap

Electron main 是"外壳"，职责边界清晰：不承载业务逻辑，只负责启动与生命周期。

```text
Electron main 启动流程：
  1. app ready → bootstrap()
     ├─ init 注入（SSH 连接管理器工厂、logger、Pi SDK 配置）
     ├─ 程序化数据库迁移（runMigrations，失败即退出）
     ├─ 预置数据（分组/主题/快速命令 seed）
     └─ 注册进程异常处理器 + 优雅关闭（刷审计缓冲）
  2. 启动打包后的 react-start server（createServerEntry，绑定 localhost 端口）
  3. 注册 ssh:// 协议处理器
  4. 加载 renderer（生产：打包产物；开发：vite dev server）
```

| 关注点 | 归属 |
| --- | --- |
| DB 迁移、预置数据、定时任务、优雅关闭 | `packages/desktop/src/bootstrap.ts` |
| HTTP 服务启动 | react-start `createServerEntry`（Nitro 打包，fsdx 同款） |
