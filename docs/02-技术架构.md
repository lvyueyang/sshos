...
| AI 对话流 | SFn POST（流式） | handler 返回 `ReadableStream<AiTextDelta>`，面板逐块消费 |
...
**AI 对话流** — 走 `aiChatSFn`（SFn 流式，handler 返回 `ReadableStream<AiTextDelta>`，服务端依赖在 handler 内动态 import），客户端 `AiPanel` 调用后逐块读取 `text-delta` 增量。`@tanstack/ai-react` 的 `useChat` 未采用（依赖保留），详见决策记录技术验证记录「SFn 流式返回实测」与 §8。
...
> AI 对话走 SFn 流式：`aiChatSFn` 返回 `ReadableStream<AiTextDelta>`（见 §5.5 / §8），`@tanstack/ai-react` 的 `useChat` 未采用，AiPanel 逐块消费增量（决策记录技术验证记录）。
...
| `ai.functions` aiChatSFn | SFn POST (stream) | AI 对话流（ReadableStream<AiTextDelta>，text-delta 增量） |
...
AI 能力通过 Pi SDK 嵌入，AI 对话由 `aiChatSFn`（SFn 流式）返回 `ReadableStream<AiTextDelta>` 增量流，客户端 `AiPanel` 逐块消费。Policy Engine 拦截 AI 生成的命令（`exec.service` 的 `execWithPolicy`）。

> **包名核实**：AI 引擎包为 `@earendil-works/pi-coding-agent`（npm 已确认存在，`exports['.']` 提供 types + import 可编程入口）。`@pi.dev/sdk` 不存在，文档/代码中严禁使用。

> **流式形态（D21 修订）**：早期 SFn 返回 `Response` 被 `startSerializer` 序列化层吞掉、async generator 无法流式，AI 对话走 Server Route SSE（`/api/ai/chat`）；D21 后官方 RawStream 帧协议使 SFn 支持流式返回，AI 对话迁移为 `aiChatSFn`（handler 返回 `ReadableStream<AiTextDelta>`，`promptGuardMiddleware` 挂中间件做注入检测）。`@tanstack/ai-react` 的 `useChat` 未采用（依赖保留）。
...
用户输入 → AiPanel 调用 aiChatSFn({ data: { sessionId, messages } })
  ...
  → ReadableStream<AiTextDelta>（text-delta 增量）→ 客户端逐块流式渲染
...
export interface AiTextDelta { type: 'text-delta'; delta: string }
...
export async function createAiChatStream(
  ...
  // 流控制器与关闭标记提升到流外：prompt 失败时也 closeStream()，避免客户端 loading 卡死
...
for (;;) {
  const { done, value } = await reader.read()
  if (done) break
  if (value?.type === 'text-delta' && value.delta) appendAssistantDelta(value.delta)
}
```
