...
└── auth.json                 # 仅占位空 {}（凭据走 master.key 加密入库，运行时注入内存）
```
...
- **凭据**：API key 加密存 `setting` 表 `ai.credential.<provider>`（master.key AES-256-GCM），`setRuntimeApiKey` 注入内存（实测不落明文），重启从 SQLite 恢复注入。
