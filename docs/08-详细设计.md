...
| `getConnectionSettingSFn` | GET | 读每连接配置（App 框架 settings 网关，key = `app.<id>.state` 及壁纸/默认终端主题等连接级设置） | 同上 |
| `setConnectionSettingSFn` | POST | 写每连接配置（upsert） | 同上 |
| `recordAuditSFn` | POST | App 框架审计记录（`ctx.audit.record` 的服务端落库通道） | 同上 |
| `connectSFn` | POST | 建立 SSH 连接，返回 sessionId；服务端按 connectionId 幂等——已有存活会话直接返回既有 sessionId（接管，不新建） | `apps/terminal/` |
| `heartbeatSFn` | POST | 会话心跳续租，返回 `{ alive }`；alive=false 表示会话已失效（服务端重启等），客户端据其降级重连 | 同上 |
| `disconnectSFn` | POST | 断开指定 SSH 连接（同步清空该会话审批挂起项 / Profile / 工具缓存 / WS 票据） | 同上 |
| `ptyWsTicketSFn` | POST | 签发 WebSocket 握手票据（绑定 sessionId，一次性、TTL 15s） | 同上 |

> **PTY 通道不走 SFn**：终端输出下行 / 输入上行 / resize 全部经 WebSocket（`server/routes/api/pty-ws.ts`，Nitro `features.websocket`）由 `@xterm/addon-attach` 承载，协议为原始终端流 + `\x1b[<rows>;<cols>R` resize 序列（决策记录「PTY 通道 WebSocket」）；`createPtySFn` / `sendInputSFn` / `resizePtySFn` / `closePtySFn` / `ptyStreamSFn` 已随迁移移除。
...
// 每连接配置（键值）：连接级设置与 App 连接属性状态（key = app.<id>.state，及壁纸/默认终端主题/快速命令作用域等连接级设置）。
// 桌面布局与 App 展示/操作态（打开的应用窗口、位姿、files 上次路径）归客户端 tab store（localStorage），不落库。
...
interface AppContext {
  ...
  audit: AuditApi
  settings: SettingsApi      // DB 通道：连接属性/偏好类状态（connection_setting）
  uiState: UiStateApi        // tab store 通道（localStorage）：展示/操作上下文类状态，刷新持久化
...
> **持久化（决策记录「会话接管与空闲回收」「App 插件框架」）**：桌面 UI 形状（tabs / activeTabId）经 zustand `persist` 中间件落 localStorage。一个连接 = 一个 Tab，**连接配置快照（`tab.connection`，建连时由 DB 回落）、窗口位姿（`tab.windows`）与 app 展示/操作上下文态（`tab.uiState`，如 files 上次打开的路径）都长在 tab 上**，随整棵 tabs 一起持久化；刷新后桌面回显，重调 `connectSFn` 由服务端幂等接管（存活会话复用 sessionId，不新建）。壳级桌面设置（壁纸/主题，`connection_setting` key = `desktop.*`）DB 权威 + 回落 store；app 内部状态（`app.<id>.state`）由 app 经 `ctx.settings` 自取自维，不回落。上方代码示例为早期形态，实现以 `stores/windows.ts` 现状为准并接入 `persist`。
...
客户端 Terminal                                web 服务端                     远程 shell（ssh2 PTY）
  addon-attach onData ──上行原始字节──▶ ┌──── WS 网关 ───────▶ PTY channel write ──▶ stdin
  addon-attach 收到下行 ──▶ write   ◀──│   （按 sessionId 绑定会话）
  resize 序列 \x1b[<rows>;<cols>R ────▶ └──────────────────── PTY resize
  Terminal.write ◀─原始终端流───────── ◀──────────────────── stdout 数据
```

**协议要点**（对齐 `@xterm/addon-attach`，成熟库承载通道）：

- **握手**：客户端先经 `ptyWsTicketSFn` 获取一次性握手票据（绑定 sessionId，TTL 15s），随后以票据建立 WebSocket 连接；服务端（Nitro 网关 `server/routes/api/pty-ws.ts`）按票据绑定到对应 PTY 会话并校验鉴权。
- **下行**（Server → Client）：原始终端流文本帧（UTF-8，服务端 StringDecoder 保证跨 chunk 多字节安全），`addon-attach` 直接 `Terminal.write`。
- **上行**（Client → Server）：原始键盘字节（`addon-attach` 的 `onData` 自动转发）；终端尺寸经标准 resize 序列 `\x1b[<rows>;<cols>R` 由客户端在 `onResize` 时发送，服务端解析后调 `setWindow`、不转发到 stdin。
- **生命周期**：`open` 钩子即建 PTY（默认 80×24，尺寸由 resize 序列校正）；连接关闭 / 远端 channel 断开自动销毁 PTY；建连期间到达的输入在服务端缓冲，PTY 就绪后冲刷（不丢首命令）。
- **发射频率**：shell 高频输出逐帧下行，不做缓冲合并（保证低延迟；本机场景实测首包 < 5ms 级）；发送缓冲超 1MB 水位时暂停 ssh2 channel，`drain` 后恢复。
