...
| 包 | 说明 |
|----|------|
| `@sshos/web` | TanStack Start 应用包（SSH 引擎 / 策略分类器内聚于 `services/`；SFn / Server Route / 桌面应用插件 / App 插件框架） |
...
| 文档 | 说明 |
|------|------|
| [项目概述](docs/01-项目概述.md) | 项目定位、核心功能、运行形态 |
| [技术架构](docs/02-技术架构.md) | 分层架构、技术栈、SFn / Server Route / WebSocket 通信、App 插件框架、AI 安全策略引擎 |
| [界面设计](docs/03-界面设计.md) | 桌面外壳、终端、文件管理、监控、AI 面板全部界面规范 |
| [决策记录](docs/决策记录.md) | 关键技术决策（按主题组织），单一事实来源 |
| [界面框图](docs/05-界面框图.md) | 全界面 ASCII 框图与标注汇总 |
| [UI 范式与规则](docs/UI范式与规则.md) | 主题 token、组件分层、图标 / 动效 / 反馈、桌面交互规则 |
| [详细设计](docs/08-详细设计.md) | 接口清单、数据模型、策略引擎与 AI 集成契约 |
| [TODO](docs/TODO.md) | 待办与待验证清单 |
